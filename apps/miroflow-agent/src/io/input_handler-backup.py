...
def process_input(task_description, task_file_name):
    ...
    if task_file_name:
        ...
        updated_task_description += f"\nNote: A {file_type} file '{task_file_name}' is associated with this task. You may use available tools to read its content if necessary.\n\n"

        logger.info(
            f"Info: Detected {file_type} file {task_file_name}, added note to description."
        )
...
class _CustomMarkdownify(markdownify.MarkdownConverter):
    ...
    def convert_img(self, el: Any, text: str, parent_tags: Any) -> str:
        ...
        title_part = ' "%s"' % title.replace('"', r"\"") if title else ""
        # parent_tags is a list of parent tag names
        if parent_tags and isinstance(parent_tags, list):
            if (
                el.parent
                and hasattr(el.parent, "name")
...
def WikipediaConverter(local_path: str, **kwargs):
    ...
    if body_elm:
        ...
        if (
            title_elm
            and isinstance(title_elm, Tag)
            and hasattr(title_elm, "string")
            and title_elm.string
        ):
...
def YouTubeConverter(local_path: str, url: str):
        ...
        for script in soup(["script"]):
            ...
            if "ytInitialData" in content:
                ...
                if obj_start >= 0 and obj_end >= 0:
                    ...
                    if (
                        attrdesc
                        and isinstance(attrdesc, dict)
                        and "content" in attrdesc
                    ):
                        content_val = attrdesc.get("content")
    ...
    if "v" in params:
        ...
        try:
            ...
            transcript_text = SRTFormatter().format_transcript(transcript)  # type: ignore
        except Exception:
            pass
    if transcript_text:
        webpage_text += f"\n### Transcript\n{transcript_text}\n"
...
def XlsxConverter(local_path: str):
    ...
    #     md_content += convert_html_to_md(html_content).text_content.strip() + "\n\n"
    ...
    for sheet_name in wb.sheetnames:
        try:
            ...
            min_row, min_col = 1, 1
            max_row = max(
                (cell.row for cell in sheet._cells.values() if cell.value is not None),  # type: ignore
                default=0,
            )
            max_col = max(
                (
                    cell.column
                    for cell in sheet._cells.values()  # type: ignore
...
def PptxConverter(local_path) -> DocumentConverterResult:
    """
    ...
    Returns:
        DocumentConverterResult with title and text_content
    """
    ...
    for slide in presentation.slides:
        ...
        for shape in slide.shapes:
            ...
            if is_table(shape):
                ...
                first_row = True
                for row in getattr(shape, "table", []).rows:  # type: ignore
                    html_table += "<tr>"
            ...
            # Text areas
                ...
                if shape == title:
                    md_content += "# " + getattr(shape, "text", "").lstrip() + "\n"
                else:
                    md_content += getattr(shape, "text", "") + "\n"
...
def _transcribe_audio(local_path) -> str:
    ...
    with sr.AudioFile(local_path) as source:
        ...
        return recognizer.recognize_google(audio).strip()  # type: ignore


def WavConverter(local_path) -> DocumentConverterResult:
    """
...
def Mp3Converter(local_path: str, extension: str, **kwargs):
    ...
    try:
        ...
        try:
            transcript = _transcribe_audio(temp_path).strip()
            md_content += "\n\n### Audio Transcript:\n" + (
                "[No speech detected]" if transcript == "" else transcript
            )
        except Exception:
...
if __name__ == "__main__":
    ...
    with open("file_to_markdown_test.txt", "w") as f:
        for file in Path(sys.argv[1]).iterdir():
            initial_user_content, updated_task_description = process_input(
                "Test file completion tests", str(file)
            )
            f.write(updated_task_description + "\n\n\n")
